...
class VertexAIService:
    ...
    def _create_tools(self) -> List[Tool]:
        ...
        return [
            Tool(
                function_declarations=[inspect_dlp, analyze_healthcare, calculate_risk]
            )
        ]
    ...
    async def detect_country(
        self, columns: List[str], sample_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Detected country: {result.get('country')} ({result.get('confidence') * 100:.0f}%)"
            )
            return result
        except Exception as e:
    ...
    async def generate_anonymization_strategy(
        self, columns: List[str], regulation: str, detected_sensitive_fields: List[str]
    ) -> Dict[str, Any]:
        """
