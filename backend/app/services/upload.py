...
class UploadService:
    ...
    async def save_file(self, file: UploadFile, dataset_id: int) -> Tuple[str, str, int]:
        """
    ...
    def detect_conflicts(self, db: Session, dataset_id: int) -> ConflictDetectionResponse:
        """
        ...
        files = db.query(UploadedFile).filter(
            UploadedFile.dataset_id == dataset_id
        ).all()
        ...
        for file in files:
            ...
            for col_name, col_type in columns_info.items():
                ...
                all_columns[col_name].append(ColumnInfo(
                    name=col_name,
                    dtype=col_type,
                    file_id=file.id,
        ...
        for col_name, col_infos in all_columns.items():
            ...
            if len(col_infos) < len(files):
                conflicts.append(ColumnConflict(
                    column_name=col_name,
                    variations=col_infos,
                    is_missing=True
                ))
    ...
    def resolve_conflicts(
        ...
        if dataset.status != DatasetStatus.PENDING_RESOLUTION:
            raise HTTPException(
                status_code=400,
                detail="Dataset is not in pending resolution state"
            )
